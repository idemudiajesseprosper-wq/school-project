    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f8f9fb", fontFamily: "Lato, sans-serif" }}>
      ...
      <main style={{ flex: 1, display: "flex", flexDirection: "column", minWidth: 0, overflowX: "hidden" }}>
        ...
        <div style={{ padding: "32px", flex: 1 }}>
            ...
            {stats.map((s) => (
              <div key={s.label} style={{ background: "white", borderRadius: "16px", padding: "20px 24px", border: "1px solid #e8edf3", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
                <div style={{ width: "38px", height: "38px", borderRadius: "10px", background: s.bg, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: "14px" }}>
                  <span style={{ color: s.color, fontSize: "18px", fontWeight: "900" }}>{s.value}</span>
                </div>
                <p style={{ color: "#6b7280", fontSize: "12px", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.08em" }}>{s.label}</p>
              </div>
            ))}
          </div>
          ...
          <div style={{ background: "white", borderRadius: "16px", border: "1px solid #e8edf3", padding: "16px 20px", marginBottom: "20px", display: "flex", alignItems: "center", gap: "16px", flexWrap: "wrap", boxShadow: "0 1px 4px rgba(0,0,0,0.04)" }}>
            <div style={{ position: "relative", flex: 1, minWidth: "240px" }}>
              ...
              <input
                ...
                style={{ width: "100%", padding: "11px 14px 11px 44px", border: "1px solid #e5e7eb", borderRadius: "10px", fontSize: "14px", outline: "none", color: "#111", background: "#f9fafb" }}
              />
            </div>
            <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
              {[
                ...
                <button key={f.key} className="filter-chip" onClick={() => setFilterStatus(f.key)} style={{ padding: "8px 16px", borderRadius: "8px", fontSize: "13px", fontWeight: "600", border: "1px solid", cursor: "pointer", transition: "all 0.15s", borderColor: filterStatus === f.key ? "#2563EB" : "#e5e7eb", background: filterStatus === f.key ? "#eff6ff" : "white", color: filterStatus === f.key ? "#2563EB" : "#6b7280" }}>
                  {f.label}
                </button>
              ))}
            </div>
          ...
          {/* Table */}
          <div style={{ background: "white", borderRadius: "20px", border: "1px solid #e8edf3", overflow: "hidden", boxShadow: "0 1px 8px rgba(0,0,0,0.05)" }}>
                ...
                <thead>
                  <tr style={{ background: "#f8f9fb", borderBottom: "2px solid #e8edf3" }}>
                    {["Student", "Role", "Status", "Online", "Login Count", "Last Login", "Joined", "Actions"].map(h => (
                      <th key={h} style={{ padding: "14px 20px", textAlign: "left", fontSize: "11px", fontWeight: "700", color: "#6b7280", textTransform: "uppercase", letterSpacing: "0.08em", whiteSpace: "nowrap" }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    <tr key={student._id} className="student-row" style={{ borderBottom: i < filtered.length - 1 ? "1px solid #f1f5f9" : "none", background: "white", transition: "background 0.15s" }}>
                      ...
                      <td style={{ padding: "16px 20px" }}>
                        <div style={{ display: "flex", alignItems: "center", gap: "14px" }}>
                          ...
                          <div>
                            <p style={{ fontWeight: "700", color: "#0a0a0a", fontSize: "14px" }}>{student.fullName}</p>
                            <p style={{ color: "#9ca3af", fontSize: "12px", marginTop: "2px" }}>{student.email}</p>
                          </div>
                        </div>
                      </td>
            ...
            {!loading && filtered.length === 0 && (
              <div style={{ padding: "80px 20px", textAlign: "center" }}>
                ...
                <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: "22px", fontWeight: "900", color: "#111", marginBottom: "8px" }}>No Students Found</h3>
                <p style={{ color: "#9ca3af", fontSize: "14px" }}>{search ? "Try adjusting your search or filters." : "No students registered yet."}</p>
              </div>
            )}
...
function ActionBtn({ onClick, disabled, color, label, icon, iconOnly }) {
  return (
    <button className="action-btn" onClick={onClick} disabled={disabled} style={{ display: "inline-flex", alignItems: "center", gap: "6px", padding: iconOnly ? "8px" : "8px 14px", borderRadius: "9px", border: "none", cursor: disabled ? "not-allowed" : "pointer", background: color + "18", color, fontSize: "12px", fontWeight: "700", opacity: disabled ? 0.5 : 1 }} title={label}>
      {icon}
      {!iconOnly && label}
    </button>
  );
