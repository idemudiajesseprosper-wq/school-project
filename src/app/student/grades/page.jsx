...
export default function StudentGradesPage() {
  ...
  const rows = useMemo(() => {
    const map = new Map(
      (data.submissions || []).map((s) => [String(s.assignmentId), s]),
    );
    return (data.assignments || []).map((a) => ({
      assignment: a,
      submission: map.get(String(a._id)),
    }));
  }, [data]);
  ...
  const pendingGradeCount = rows.filter(
    (r) => r.submission && !r.submission.isGraded,
  ).length;
    ...
    <div style={S.page} className="portal-page">
      ...
      <main className="portal-main" style={S.main}>
        ...
        <header style={S.header}>
          ...
          <a href="/student/assignments" style={S.actionBtn}>
            Open Assignments →
          </a>
        </header>
        ...
        <div style={S.tableCard}>
          ...
          {/* Rows */}
          {rows.length > 0 ? (
            rows.map(({ assignment, submission }, i) => (
              <div
                key={assignment._id}
                className="grade-row"
            ...
            <div style={{ padding: "56px 20px", textAlign: "center" }}>
              ...
              <p style={{ fontSize: 13, color: T.t3 }}>
                Grades will appear here after teachers post and grade
                assignments.
              </p>
            </div>
          )}
