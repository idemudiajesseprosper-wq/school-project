...
export default function StudentResultsPage() {
  const [filters, setFilters] = useState({ academicSession: currentSession(), term: "First Term" });
      ...
      <main style={s.main}>
        ...
        <section style={s.filterCard}>
          ...
          <div style={s.filterFields}>
            ...
            <button
              disabled={loading}
              style={{ ...s.viewBtn, ...(loading ? { opacity: 0.7, cursor: "not-allowed" } : {}) }}
              onClick={loadResult}
        ...
        {!result && !loading && (
          <div style={s.emptyState}>
            ...
            <p style={{ fontFamily: "'Sora',sans-serif", fontSize: 15, fontWeight: 700, color: "#94a3b8", margin: 0 }}>No result loaded</p>
            <p style={{ fontSize: 13, color: "#cbd5e1", margin: 0, textAlign: "center", maxWidth: 280, lineHeight: 1.6 }}>
              Select a session and term above, then click <strong style={{ color: "#2563eb" }}>View Results</strong>.
            </p>
          </div>
        )}
    ...
    <section id="report-card" style={s.reportCard}>
        ...
        <div style={s.passportWrap}>
          {student.passport
            ...
            : <div style={s.passportPlaceholder}>
                <svg width="22" height="22" fill="none" stroke="#cbd5e1" strokeWidth="1.5" viewBox="0 0 24 24">
            ...
            <tbody>
              {student.subjects.map((item, i) => {
                ...
                return (
                  ...
                  >
                    <td style={{ ...s.td, fontWeight: 600, color: "#0f172a" }}>{item.subject}</td>
                    <td style={{ ...s.td, color: "#64748b" }}>{item.caScore}</td>
                    <td style={{ ...s.td, color: "#64748b" }}>{item.examScore}</td>
                    <td style={{ ...s.td, fontWeight: 700, color: "#0f172a" }}>{item.totalScore}</td>
                    <td style={s.td}>
                      <span style={{ ...s.gradeChip, color: gradeColor(item.grade), background: gradeColor(item.grade) + "12", border: `1px solid ${gradeColor(item.grade)}30` }}>
                        {item.grade}
                      </span>
                    </td>
            ...
            return (
              <div key={item.subject} style={{ ...s.subjectCard, animationDelay: `${i * 35}ms` }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  <span style={{ fontWeight: 700, fontSize: 13, color: "#0f172a" }}>{item.subject}</span>
                  <span style={{ ...s.gradeChip, color: gradeColor(item.grade), background: gradeColor(item.grade) + "12", border: `1px solid ${gradeColor(item.grade)}30` }}>
                    {item.grade}
                  </span>
                </div>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", marginTop: 6 }}>
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>CA: <strong style={{ color: "#475569" }}>{item.caScore}</strong></span>
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>Exam: <strong style={{ color: "#475569" }}>{item.examScore}</strong></span>
                  <span style={{ fontSize: 11, color: "#94a3b8" }}>Total: <strong style={{ color: "#0f172a" }}>{item.totalScore}</strong></span>
                </div>
              </div>
            );
          })}
      ...
      <div className="rc-summary-grid" style={s.summaryGrid}>
        {[
          ...
          ["Attendance", student.attendance || "N/A", "#7c3aed"],
...
function BookIcon() {
  return <svg width="14" height="14" fill="none" stroke="#2563eb" strokeWidth="2" viewBox="0 0 24 24" style={{ flexShrink: 0 }}><path d="M4 19.5A2.5 2.5 0 016.5 17H20M4 19.5A2.5 2.5 0 014 17V5a2 2 0 012-2h14v14H6.5A2.5 2.5 0 004 19.5z" strokeLinecap="round" strokeLinejoin="round" /></svg>;
}

const s = {
  root: { fontFamily: "'DM Sans', sans-serif", minHeight: "100vh", background: "#f1f5f9", color: "#0f172a" },

  // Header
  header: { background: "#ffffff", borderBottom: "1px solid #e2e8f0", boxShadow: "0 1px 4px rgba(0,0,0,0.06)" },
  headerInner: {
  ...
  portalBadge: {
    ...
    border: "1px solid #bfdbfe", borderRadius: 20, padding: "3px 10px", marginBottom: 8,
  },
  heading: { fontFamily: "'Sora', sans-serif", fontSize: 24, fontWeight: 800, color: "#0f172a", margin: "0 0 4px", lineHeight: 1.2 },
  subheading: { fontSize: 13, color: "#94a3b8", margin: 0 },
  backBtn: {
    display: "flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 700,
    color: "#64748b", background: "#f8fafc", border: "1px solid #e2e8f0",
    borderRadius: 8, padding: "8px 14px", textDecoration: "none", whiteSpace: "nowrap", flexShrink: 0,
  },
  ...
  // Filter
  filterCard: {
    background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "18px 20px",
    display: "flex", alignItems: "flex-end", gap: 14, boxShadow: "0 1px 6px rgba(0,0,0,0.05)",
  ...
  label: { fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#94a3b8" },
  input: {
  ...
  emptyIconWrap: {
    width: 64, height: 64, borderRadius: 18, background: "#f8fafc",
  ...
  // Report card
  reportCard: {
    background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: 16, padding: "20px",
    boxShadow: "0 1px 6px rgba(0,0,0,0.05)", animation: "fadeUp 0.4s ease both",
  },
  actionBar: { display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" },
  pdfBtn: {
    display: "flex", alignItems: "center", gap: 7, padding: "10px 18px",
    borderRadius: 10, fontSize: 13, fontWeight: 700, cursor: "pointer",
    background: "#0f172a", color: "#f59e0b", border: "none",
  ...
  // School header
  schoolHeader: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16, marginBottom: 18 },
  logoWrap: {
    width: 52, height: 52, borderRadius: 12, overflow: "hidden",
    background: "#f8fafc", border: "1px solid #e2e8f0", flexShrink: 0, padding: 4,
  },
  schoolName: { fontFamily: "'Sora', sans-serif", fontSize: 18, fontWeight: 800, color: "#0f172a", margin: "0 0 2px" },
  reportLabel: { fontSize: 12, color: "#94a3b8", margin: "0 0 6px" },
  sessionTag: {
    display: "inline-block", fontSize: 11, fontWeight: 600, color: "#2563eb",
    background: "#eff6ff", border: "1px solid #bfdbfe", borderRadius: 20, padding: "2px 10px",
  },
  passportWrap: {
    width: 72, height: 80, borderRadius: 10, overflow: "hidden",
    border: "2px solid #e2e8f0", background: "#f8fafc", flexShrink: 0,
  },
  passportPlaceholder: { width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center" },
  ...
  infoLabel: { fontSize: 10, fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", color: "#94a3b8" },
  infoValue: { fontSize: 14, fontWeight: 800, color: "#0f172a", fontFamily: "'Sora', sans-serif" },
  ...
  tableWrap: { borderRadius: 10, border: "1px solid #e2e8f0", overflow: "hidden" },
  table: { width: "100%", borderCollapse: "collapse", fontSize: 13 },
  th: {
    background: "#f8fafc", padding: "10px 14px", textAlign: "left",
    fontSize: 10, fontWeight: 700, letterSpacing: "0.09em",
    textTransform: "uppercase", color: "#94a3b8", borderBottom: "1px solid #e2e8f0",
  },
  ...
  barBg: { height: 5, borderRadius: 99, background: "#e2e8f0", overflow: "hidden" },
  ...
  summaryLabel: { fontSize: 10, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#94a3b8" },

  // Remarks
  remarkGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 },
  remarkBox: { background: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: 12, padding: "14px 16px" },
  remarkHeader: { display: "flex", alignItems: "center", gap: 7, marginBottom: 10 },
  remarkTitle: { fontSize: 11, fontWeight: 700, letterSpacing: "0.07em", textTransform: "uppercase", color: "#94a3b8" },
