...
export default function Apply() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      payload.passport = await uploadFile(
        formData,
        "passport",
        "school-portal/applications/passports",
      );
      payload.birthCertificate = await uploadFile(
        formData,
        "birthCertificate",
        "school-portal/applications/documents",
      );
      payload.previousSchoolResult = await uploadFile(
        formData,
        "previousSchoolResult",
        "school-portal/applications/documents",
      );
      payload.transferCertificate = await uploadFile(
        formData,
        "transferCertificate",
        "school-portal/applications/documents",
      );
      ...
      const json = await res.json();
      if (!json.success)
        return toast.error(json.message || "Could not submit application");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-red-50 p-8 text-red-950">
        Loading application...
      </div>
    );
  }
      ...
      <section className="mx-auto max-w-5xl overflow-hidden rounded-2xl border border-red-100 bg-white shadow-xl">
        <div className="bg-red-900 px-6 py-7 text-white">
          ...
          <p className="mt-2 text-sm text-red-100">
            Applicant ID: {applicant.applicantId}. Complete the form first, then
            pay the enrollment fee from your dashboard.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="grid gap-7 p-5 md:p-7">
          ...
          <Section title="Medical Information">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <Input
                ...
                label="Special Attention Needed"
                full
              />
            </div>
          </Section>
          ...
          <Section title="Undertaking">
            <div className="rounded-xl border border-red-100 bg-red-50 p-4 text-sm leading-7 text-slate-700">
              <p>
                I confirm that all information provided is accurate. I undertake
                to comply with the rules and regulations of Winners' Foundation
                School.
              </p>
            </div>
          ...
          <button
            disabled={submitting}
            className="rounded-xl bg-red-700 px-5 py-4 text-sm font-black uppercase tracking-wider text-white hover:bg-red-800 disabled:opacity-60"
          >
...
function Section({ title, children }) {
  return (
    <section>
      <h2 className="mb-4 border-b border-red-100 pb-2 text-lg font-black text-red-950">
        {title}
      </h2>
      {children}
    </section>
...
function Input({ label, name, type = "text", required, full, defaultValue }) {
  return (
    <label
      className={`grid gap-1 text-xs font-black uppercase tracking-wider text-slate-500 ${full ? "md:col-span-2" : ""}`}
    >
      {label}
      <input
        name={name}
        ...
        className="rounded-lg border border-slate-200 bg-slate-50 px-3 py-3 text-sm font-medium normal-case tracking-normal text-slate-950 outline-none focus:border-red-500 focus:bg-white focus:ring-4 focus:ring-red-100"
      />
    </label>
  );
}
