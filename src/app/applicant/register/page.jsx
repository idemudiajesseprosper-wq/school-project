...
export default function ApplicantRegisterPage() {
  ...
  async function submit(e) {
    ...
    setLoading(false);
    if (!json.success) return toast.error(json.message || "Registration failed");
    toast.success("Account created. Check your email to verify.");
    router.push("/applicant/login");
  }

  return (
    <AuthShell title="Applicant Enrollment" subtitle="Create your account to begin admission.">
      <form onSubmit={submit} className="grid gap-3">
        ...
        <button disabled={loading} className="auth-btn">{loading ? "Creating..." : "Create Applicant Account"}</button>
      </form>
      <p className="mt-4 text-center text-sm text-red-100/70">
        Already registered? <a className="font-bold text-white" href="/applicant/login">Login</a>
      </p>
    </AuthShell>
  );
}
    ...
    <main className="min-h-screen bg-red-950 px-4 py-20 text-white">
      ...
      <section className="mx-auto max-w-md rounded-2xl border border-white/15 bg-white/10 p-6 shadow-2xl backdrop-blur">
        <div className="mb-6 text-center">
          ...
          <p className="text-xs font-black uppercase tracking-[0.2em] text-red-100">Winners' Foundation School</p>
